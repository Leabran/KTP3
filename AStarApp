...
/**
 ...
 Пользователь может нарисовать или очистить препятствия на карте, а затем нажать кнопку, чтобы вычислить
 путь от начала до конца, используя алгоритм поиска путей A *.
 Если путь найден, то он отображается зеленым цветом.
 **/
public class AStarApp {

    /** Количество ячеек сетки в направлении Х **/
    private int width;
    ...
    private class MapCellHandler implements MouseListener
    {
        ...
        /** Инициация операции модификации. **/
    ...
    /** Создает новый экземпляр AStarApp с указанной шириной карты и высота. **/
